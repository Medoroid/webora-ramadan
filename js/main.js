...
// تشغيل الموسيقى بعد أول click لتجاوز حظر المتصفح
function startMusicOnce() {
  if (musicSwitch.checked) {
    vibs.play().catch(e => console.log("Audio blocked:", e));
  }
  document.removeEventListener("click", startMusicOnce);
}
document.addEventListener("click", startMusicOnce);

// Switch للتحكم في الصوت بسرعة
...
function createHeart(emoji, className) {
  ...
  el.animate([{ transform: "translateY(0)", opacity: 1 }, { transform: "translateY(-110vh)", opacity: 0 }], { duration: 6000, easing: "linear" });
